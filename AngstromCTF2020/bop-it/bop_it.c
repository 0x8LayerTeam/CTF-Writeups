...
int main() {
	...
	char c;
	char *action = actions[rand()%4]; //choose action message to be printed
	write(1, action, strlen(action)); //printf action
	while ((c = getchar()) != EOF) {  
		if (!strcmp(action, actions[3])) { //while action == "flag it" (action[3])
			char guess[256];
			guess[0] = c; //guess[0] == our input
